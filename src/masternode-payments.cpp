...
CCriticalSection cs_vecMasternodePayments;
...
void CMasternodePayments::ProcessMessageMasternodePayments(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "mnget") { //Masternode Payments Request Sync
        ...
        int nFirstBlock = nHeight - (m_nodeman.CountEnabled() * 1.25);
        ...
        if (!winner.SignatureValid()) {
            ...
            m_nodeman.AskForMN(pfrom, winner.vinMasternode);
            return;
        }
...
bool CMasternodePaymentWinner::Sign(CKey& keyMasternode, CPubKey& pubkey2)
{
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    if (IsSporkActive(SPORK_23_MASTERNODE_PAYMENT_ENFORCEMENT)) {
        // Get a stable number of masternodes by ignoring newly activated (< 8000 sec old) masternodes
        nMasternode_Drift_Count = m_nodeman.stable_size() + Params().MasternodeCountDrift();
    }
    else {
        ...
        nMasternode_Drift_Count = m_nodeman.size() + Params().MasternodeCountDrift();
    }

    CAmount requiredMasternodePayment = GetMasternodePayment(nBlockHeight, nReward, nMasternode_Drift_Count, false);
...
void CMasternodePayments::CleanPaymentList()
{
    ...
    int nLimit = std::max(int(m_nodeman.size() * 1.25), 1000);
...
{
    CMasternode* pmn = m_nodeman.Find(vinMasternode);
    ...
    if (!pmn) {
        ...
        m_nodeman.AskForMN(pnode, vinMasternode);
        return false;
    }
    ...
    int n = m_nodeman.GetMasternodeRank(vinMasternode, nBlockHeight - 100, ActiveProtocol());
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight)
{
    ...
    int n = m_nodeman.GetMasternodeRank(activeMasternode.vin, nBlockHeight - 100, ActiveProtocol());
    ...
    if (budget.IsBudgetPaymentBlock(nBlockHeight)) {
        ...
        int nCount = 0;
        CMasternode* pmn = m_nodeman.GetNextMasternodeInQueueForPayment(nBlockHeight, true, nCount);
    ...
    if (!obfuScationSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMasternode, pubkey2)) {
    ...
    if (newWinner.Sign(keyMasternode, pubkey2)) {
...
void CMasternodePayments::Sync(CNode* node, int nCountNeeded)
{
    ...
    int nCount = (m_nodeman.CountEnabled() * 1.25);
