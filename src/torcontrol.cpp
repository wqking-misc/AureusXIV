    ...
    reconnect_timeout(RECONNECT_TIMEOUT_START)
{
    ...
    if (!conn.Connect(_target, boost::bind(&TorController::connected_cb, this, boost::arg<1>()),
         boost::bind(&TorController::disconnected_cb, this, boost::arg<1>()) )) {
...
void TorController::auth_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        _conn.Command(strprintf("ADD_ONION %s Port=%i,127.0.0.1:%i", private_key, GetListenPort(), GetListenPort()),
            boost::bind(&TorController::add_onion_cb, this, boost::arg<1>(), boost::arg<2>()));
    } else {
...
void TorController::authchallenge_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
    if (reply.code == 250) {
        ...
        if (l.first == "AUTHCHALLENGE") {
            ...
            _conn.Command("AUTHENTICATE " + HexStr(computedClientHash), boost::bind(&TorController::auth_cb, this, boost::arg<1>(), boost::arg<2>()));
        } else {
    ...
    if (reply.code == 250) {
        ...
        if (!torpassword.empty()) {
            if (methods.count("HASHEDPASSWORD")) {
                ...
                _conn.Command("AUTHENTICATE \"" + torpassword + "\"", boost::bind(&TorController::auth_cb, this, boost::arg<1>(), boost::arg<2>()));
            } else {
                LogPrintf("tor: Password provided with -torpassword, but HASHEDPASSWORD authentication is not available\n");
            }
        } else if (methods.count("NULL")) {
            LogPrint("tor", "tor: Using NULL authentication\n");
            _conn.Command("AUTHENTICATE", boost::bind(&TorController::auth_cb, this, boost::arg<1>(), boost::arg<2>()));
            ...
            if (status_cookie.first && status_cookie.second.size() == TOR_COOKIE_SIZE) {
                // _conn.Command("AUTHENTICATE " + HexStr(status_cookie.second), boost::bind(&TorController::auth_cb, this, boost::arg<1>(), boost::arg<2>()));
                cookie = std::vector<uint8_t>(status_cookie.second.begin(), status_cookie.second.end());
                clientNonce = std::vector<uint8_t>(TOR_NONCE_SIZE, 0);
                GetRandBytes(&clientNonce[0], TOR_NONCE_SIZE);
                _conn.Command("AUTHCHALLENGE SAFECOOKIE " + HexStr(clientNonce), boost::bind(&TorController::authchallenge_cb, this, boost::arg<1>(), boost::arg<2>()));
            } else {
...
void TorController::connected_cb(TorControlConnection& _conn)
{
    ...
    if (!_conn.Command("PROTOCOLINFO 1", boost::bind(&TorController::protocolinfo_cb, this, boost::arg<1>(), boost::arg<2>())))
...
void TorController::Reconnect()
{
     ...
     * may be restarting.
     */
    if (!conn.Connect(target, boost::bind(&TorController::connected_cb, this, boost::arg<1>()),
         boost::bind(&TorController::disconnected_cb, this, boost::arg<1>()) )) {
