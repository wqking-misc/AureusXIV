...
void CObfuscationPool::CheckFinalTransaction()
{
    ...
    LOCK2(cs_main, pwalletMain->cs_wallet);
    {
        ...
        if (!CMessageSigner::GetKeysFromSecret(strFundamentalNodePrivKey, key2, pubkey2)) {
            LogPrintf("%s : Invalid fundamentalnode key %s", __func__, strFundamentalNodePrivKey);
            return;
        }
...
bool CObfuscationQueue::Sign()
{
    ...
    if (!CMessageSigner::GetKeysFromSecret(strFundamentalNodePrivKey, key2, pubkey2)) {
        return error("%s : Invalid fundamentalnode key", __func__);
    }
...
void ThreadCheckObfuScationPool()
{
    ...
    while (true) {
        ...
        if (fundamentalnodeSync.IsBlockchainSynced()) {
            ...
            if (c % 60 == 0) {
                fnodeman.CheckAndRemove();
                fnodeman.ProcessFundamentalnodeConnections();
